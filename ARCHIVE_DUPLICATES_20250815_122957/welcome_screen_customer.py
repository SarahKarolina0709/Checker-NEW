...
class WelcomeScreenCustomer:
    ...
    def _create_simple_customer_card(self, parent, column):
        ...
        # Status Section - Single Responsibility
    ...
    def _setup_customer_status_section(self, content):
        """STATUS SECTION - Aktueller Kunde und Status (volle Fläche bei 'Kein Kunde')"""
        status_frame = ctk.CTkFrame(
            content,
            fg_color=self.parent.get_color('background'),
            corner_radius=self.parent.get_component_value('borders.radius_md') if hasattr(self.parent, 'get_component_value') else 6,
        )
        ...
        current_label.pack(anchor="w", pady=(0, self.parent.get_spacing('xs')))

        # Vollflächige Status-Karte
        self.status_card = ctk.CTkFrame(
            status_content,
            fg_color=self.parent.get_color('surface_elevated'),
            border_color=self.parent.get_color('surface_border'),
            border_width=1,
            corner_radius=self.parent.get_component_value('borders.radius_lg') if hasattr(self.parent, 'get_component_value') else 10,
        )
        ...
        inner.pack(fill="both", expand=True, padx=self.parent.get_spacing('md'), pady=self.parent.get_spacing('sm'))

        # Status-Text (wird per Helper eingefärbt)
        self.customer_status_label = ctk.CTkLabel(
            inner,
            text=self.current_customer if self.current_customer else "Kein Kunde ausgewählt",
            font=ctk.CTkFont(*self.parent.get_font('heading_md')),
            text_color=self.parent.get_color('gray_700'),
        )
        self.customer_status_label.pack(anchor="w")

        # Initialen Stil anwenden
        self._apply_customer_status_style()
    ...
    def _select_customer_from_list(self, customer_name):
        ...
        try:
            ...
            # 🔓 Enable buttons when customer is selected
            self._update_customer_buttons_state(customer_selected=True)
    ...
    def _update_customer_buttons_state(self, customer_selected=None):
        """🔒 Update button states based on customer selection"""
        try:
            if customer_selected is None:
                customer_selected = bool(getattr(self, 'current_customer', None))
            
            # Update folder button
            if hasattr(self.parent, 'folder_btn') and self.parent.folder_btn:
                if customer_selected:
                    self.parent.folder_btn.configure(
                        state="normal", 
                        **self.parent._button_style('primary', 'md', 'solid')
                    )
                    if hasattr(self.parent, '_hide_tooltip'):
                        self.parent._hide_tooltip(self.parent.folder_btn)
                else:
                    self.parent.folder_btn.configure(
                        state="disabled",
                        **self.parent._button_style('inactive', 'md', 'solid')
                    )
            
            # Update remove button
            if hasattr(self.parent, 'remove_btn') and self.parent.remove_btn:
                if customer_selected:
                    self.parent.remove_btn.configure(
                        state="normal",
                        **self.parent._button_style('warning', 'md', 'solid')
                    )
                    if hasattr(self.parent, '_hide_tooltip'):
                        self.parent._hide_tooltip(self.parent.remove_btn)
                else:
                    self.parent.remove_btn.configure(
                        state="disabled",
                        **self.parent._button_style('inactive', 'md', 'solid')
            ...
            if hasattr(self.parent, 'upload_btn') and self.parent.upload_btn:
                if customer_selected:
                    self.parent.upload_btn.configure(
                        state="normal",
                        **self.parent._button_style('primary', 'md', 'solid')
                    )
                    if hasattr(self.parent, '_hide_tooltip'):
                        self.parent._hide_tooltip(self.parent.upload_btn)
                else:
                    self.parent.upload_btn.configure(
                        state="disabled",
                        **self.parent._button_style('inactive', 'md', 'solid')
    ...
    def _add_customer_working(self):
        ...
        try:
            ...
            if self.customer_manager:
                # Use business logic (supports tuple return)
                result = self.customer_manager.add_customer(customer_name)
                ...
                if ok:
                    ...
                    if msg:
                        self.parent.show_toast(msg, "warning")
    ...
    def _customer_exists(self, customer_name):
        """✅ Check if customer already exists (Manager-first, mit Legacy-Fallback)"""
        try:
            # 1) Bevorzugt: Business-Logik nutzen
            if getattr(self, 'customer_manager', None):
                try:
                    exists, _matched, _score = self.customer_manager.customer_exists(customer_name)
                    return bool(exists)
                except Exception as me:
                    print(f"⚠️ Manager exists fallback: {me}")

            # 2) Fallback: Legacy lokale Liste prüfen
            ...
            print(f"❌ Customer exists check error: {e}")
            return False
    ...
    def _confirm_customer_removal(self, customer_name, dialog):
        ...
        try:
            ...
            if self.customer_manager:
                # Use business logic (tuple or bool)
            ...
            if self.current_customer == customer_name:
                self.current_customer = None
                self._update_customer_status_display()
                # 🔒 Disable buttons when no customer is selected
                self._update_customer_buttons_state(customer_selected=False)
    ...
    def _on_customer_search(self, event=None):
        ...
        try:
            ...
            # Perform fuzzy search with minimum score threshold
    ...
    def _on_search_focus_in(self, event=None):
        """🔍 Handle search field focus - only show results if search text exists"""
        try:
            search_text = self.customer_search_entry.get().strip()
            
            # Only show results if there's actual search text
    ...
    def _fuzzy_search_customers(self, search_text):
        ...
        # 1) Bevorzugt: CustomerManager nutzen
        if getattr(self, 'customer_manager', None):
            try:
                mgr_results = self.customer_manager.search_customers(search_text, limit=8)
                norm = []
                for item in mgr_results or []:
                    if isinstance(item, dict):
                        name = item.get('name') or item.get('kundenname') or item.get('title')
                        score = int(item.get('score', item.get('bewertung', 0) or 0))
                        # Höhere Mindest-Score-Schwelle für bessere Relevanz
                        if name and score >= 25:  # Nur Ergebnisse ab 25% Ähnlichkeit
                            norm.append({'name': name, 'score': score})
                    elif isinstance(item, str):
                        norm.append({'name': item, 'score': 100})
                norm.sort(key=lambda x: x.get('score', 0), reverse=True)
                return norm
            except Exception as e:
                print(f"⚠️ Manager fuzzy search fallback: {e}")

        # 2) Legacy-Fallback: lokale Heuristik
        if not self.customers_data:
            return []
        ...
        for customer in self.customers_data:
            ...
            # Höhere Mindest-Score-Schwelle für bessere Relevanz
            if score >= 30:  # Nur Ergebnisse ab 30% Ähnlichkeit anzeigen
                results.append({'name': customer_name, 'score': int(score), 'highlight': self._get_highlight_info(search_text_lower, customer_name.lower())})
        results.sort(key=lambda x: x['score'], reverse=True)
        return results[:8]  # Maximal 8 Ergebnisse
    ...
    def _create_search_result_item(self, parent, result, index):
        """🔍 Create search result item with score display"""
        # Zeige Score nur bei < 100 (nicht bei exakten Treffern)
    ...
    def _update_customer_status_display(self):
        ...
        try:
            ...
            # Karte einfärben/neutralisieren
            self._apply_customer_status_style()
        except Exception as e:
            print(f"⚠️ Status update error: {e}")

    def _apply_customer_status_style(self):
        """Wendet Styleguide-Farben an:
        - Kein Kunde: volle Fläche warning (Orange) + weiße Schrift
        - Mit Kunde: neutrale Fläche, Erfolgstext
        """
        try:
            # Wenn die Status-UI nicht existiert (weil nicht aufgebaut), still überspringen
            if not getattr(self, 'status_card', None) or not getattr(self, 'customer_status_label', None):
                return
            has_customer = bool(self.current_customer)
            if has_customer:
                # Neutraler Card-Background, grüner Text
                ...
                self.customer_status_label.configure(
                    text_color=self.parent.get_color('success')
                )
            else:
                # Vollflächig Orange mit weißer Schrift
                self.status_card.configure(
                    fg_color=self.parent.get_color('warning'),
                    border_color=self.parent.get_color('warning'),
                    border_width=1,
                )
                self.customer_status_label.configure(
                    text_color=self.parent.get_color('white')
