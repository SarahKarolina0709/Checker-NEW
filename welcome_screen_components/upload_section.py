...
class UploadSection(ctk.CTkFrame, SectionHeaderMixin):
    ...
    def create_widgets(self):
        ...
        header_frame, icon_bg = self.create_section_header(
            ...
            icon_name="upload",
            icon_bg_color=enhanced_theme.get_color("info"),
            icon_emoji_fallback="📤"
        )
        
        # Add color animation to header
        def animate_header_color():
            try:
                colors = [
                ...
                def cycle_colors(step=0):
                    try:
                        color = colors[step % len(colors)]
                        icon_bg.configure(fg_color=color)
                        icon_bg.after(2000, lambda: cycle_colors(step + 1))
                    except:
                        pass
        ...
        icon_bg.after(1000, animate_header_color)

        # Professional drag & drop area with animations
        self.dnd_frame = ctk.CTkFrame(
            upload_container,
            fg_color=enhanced_theme.get_color("info_surface"),
            border_width=2,
            border_color=enhanced_theme.get_color("info"),
        ...
        # Add border color animation to drag & drop area
        def animate_dnd_border():
            try:
                colors = [
                    enhanced_theme.get_color('info'),
                    enhanced_theme.get_color('secondary'),
                ...
                def cycle_border_colors(step=0):
                    try:
                        color = colors[step % len(colors)]
                        self.dnd_frame.configure(border_color=color)
                        self.dnd_frame.after(1500, lambda: cycle_border_colors(step + 1))
                    except:
                        pass
        ...
        self.dnd_frame.after(1500, animate_dnd_border)
    ...
    def _create_professional_upload_content(self):
        ...
        if upload_icon:
            ...
            dnd_icon_label = ctk.CTkLabel(
                ...
                text_color=enhanced_theme.get_color("primary")
            )
            dnd_icon_label.pack(pady=(0, UITheme.SPACING_S))
    ...
    def _setup_professional_drag_drop(self):
        ...
        def on_drag_enter(event):
            self.dnd_frame.configure(
                fg_color=enhanced_theme.get_color("primary_surface"),
