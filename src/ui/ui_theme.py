...
class ProfessionalColors:
    ...
    PRIMARY_BLUE = "#0078D4"           # Microsoft Blue (more vibrant)
...
class ProfessionalGradients:
    ...
    PRIMARY_GRADIENT = "linear-gradient(135deg, #0078D4 0%, #106EBE 100%)"
...
class EnhancedUITheme:
    ...
    def _init_default_themes(self) -> None:
        ...
        light_theme = ColorScheme(
            ...
            icon="#6C757D",
            icon_light="#8E9BAE",
            icon_accent="#0078D4",     # Microsoft Blue
    ...
    def switch_to_dark_mode(self) -> None:
        """Switch current theme to its dark variant."""
        current_base = self._get_current_theme_base()
        dark_variant = f"{current_base}_dark" if current_base != "dark" else "dark"
        
        if dark_variant in self._themes:
            self.switch_theme(dark_variant)
        elif "dark" in self._themes:
...
class UITheme(metaclass=DynamicThemeMeta):
    ...
    ⚠️  WICHTIGE HINWEISE ZUR THEME-NUTZUNG  ⚠️
    
    1. STATISCHE VS. DYNAMISCHE EIGENSCHAFTEN:
       Die Eigenschaften dieser Klasse (COLOR_PRIMARY, TUPLE_BG, etc.) werden 
       dynamisch ausgewertet und geben immer die aktuellen Theme-Farben zurück.
       
       Wenn diese Werte jedoch Variablen zugewiesen werden, werden sie STATISCH:
       
       # ❌ SCHLECHT - Wird nach Theme-Wechsel nicht aktualisiert:
       my_color = UITheme.COLOR_PRIMARY
       
       # ✅ GUT - Direkter Zugriff für dynamisches Verhalten:
       widget.configure(fg_color=UITheme.COLOR_PRIMARY)
    
    2. EMPFOHLENE MODERNE API:
       Für garantierte Theme-Updates sollte neuer Code die moderne API verwenden:
       
       # ✅ BESSER - Moderne API für garantierte Theme-Updates:
       widget.configure(fg_color=enhanced_theme.get_color('primary'))
       
       # Farbtupel für Light/Dark-Mode:
       enhanced_theme.get_color_tuple('primary')  # -> (light_color, dark_color)
    
    3. LEGACY-KONSTANTEN:
       Die alten TUPLE_* und COLOR_* Konstanten werden aus Kompatibilitätsgründen
       weiterhin unterstützt, sind aber intern auf die neue Theme-API umgestellt.
       
       Für neue Entwicklungen bitte immer die moderne API verwenden.
    """
    ...
    SECTION_CONTAINER_HEIGHT = 600  # Einheitliche Höhe für alle Sektionen
    ...
    def add_keyboard_drag_drop_support(cls, widget: Any, on_files_callback: Optional[Callable[[Tuple[str, ...]], None]] = None) -> None:
        ...
        try:
            ...
            widget._accessible_description = "Dateien hierher ziehen oder Enter/Leertaste für Dateiauswahl drücken"
