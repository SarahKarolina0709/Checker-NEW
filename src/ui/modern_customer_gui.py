...
class ModernCustomerGUI(ctk.CTkFrame):
    ...
    def _create_customer_card(self, customer_name: str, row: int):
        ...
        try:
            ...
            if project_count > 0:
                ...
                no_project_badge = ctk.CTkLabel(
                    info_container,
                    text="📋 Keine Projekte",
                    font=ctk.CTkFont(size=12),
                    fg_color=["#f3f4f6", "#6b7280"],
                    text_color=["#6b7280", "#e5e7eb"],
                    corner_radius=15,
                    padx=12,
                    pady=6
                )
                no_project_badge.pack(side="left", padx=(0, 8))
        except:
        ...
        try:
            folder_btn = ctk.CTkButton(
                ...
                command=lambda: self._open_customer_folder(customer_name)
            )
    ...
    def _get_customer_projects(self, customer_name: str) -> List[str]:
        ...
        try:
            # Hier könnte eine echte Implementierung stehen
            # Für jetzt geben wir eine Demo-Liste zurück
...
• Status: {'Sehr aktiv' if recent_activity > total_customers*0.7 else 'Moderat aktiv' if recent_activity > total_customers*0.3 else 'Wenig aktiv'}
    ...
    def _edit_customer(self, customer_name: str):
        ...
        try:
            messagebox.showinfo(
                "Kunde bearbeiten",
                f"Bearbeitungsfunktion für '{customer_name}' wird in Kürze verfügbar sein."
            )
        except Exception as e:
    ...
    def _open_customer_folder(self, customer_name: str):
        ...
        try:
            customer_folder = self.kunden_manager.kunden_ordner(customer_name)
            if os.path.exists(customer_folder):
                subprocess.run(['explorer', customer_folder], check=True)
            else:
                messagebox.showwarning(
                    "Ordner nicht gefunden",
                    f"Der Ordner für '{customer_name}' wurde nicht gefunden:\n{customer_folder}"
                )
        except Exception as e:
