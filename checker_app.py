...
# Customer Management - Single source of truth
# ModernCustomerGUI is imported dynamically when needed
...
class CheckerApp:
    ...
    def __init__(self) -> None:
        ...
        self.enhanced_ui: Optional[Any] = None
        self.ui_modernizer: Optional[ModernUIUpdater] = None
        
        # Core managers
        self.kunden_manager: Optional[KundenManager] = None
        self.kunden_manager_v2: Optional[KundenManagerV2] = None
    ...
    def _init_core_system(self):
        ...
        try:
            ...
            # Initialize managers - IMPORTANT: This was missing!
            self._init_managers()
    ...
    def _init_managers(self):
        ...
        try:
            ...
            print(f"[CRITICAL ERROR] Error initializing managers: {e}")
            import traceback
            traceback.print_exc()
            ...
            if hasattr(self, 'logger') and self.logger:
                ...
                print(f"[MANAGERS] Error initializing managers (no logger): {e}")
            raise
    ...
    def _init_application(self):
        ...
        try:
            ...
            # Robust error handling - check if error_monitor exists
            if hasattr(self, 'error_monitor') and self.error_monitor:
                ...
                # Fallback error handling if error_monitor not available
                print(f"[CRITICAL ERROR] Application initialization failed: {e}")
                import traceback
                traceback.print_exc()
                # Try to show error dialog if root exists
    ...
    def _show_simple_customer_menu(self):
        """Fallback to customer menu - redirects to ModernCustomerGUI."""
        print("[DEBUG] _show_simple_customer_menu called - redirecting to ModernCustomerGUI")
        try:
            # Direkte Umleitung zur ModernCustomerGUI anstatt des alten Tkinter-Menüs
            self.show_customer_menu()
        except Exception as e:
            print(f"[DEBUG] Final fallback failed: {e}")
            # Letzter Fallback: Einfaches Messagebox
            messagebox.showinfo(
                "Kundenverwaltung", 
    ...
    def create_new_customer(self):
        """Creates a new customer - delegated to customer management."""
        try:
            if hasattr(self, 'ui_modernizer') and self.ui_modernizer:
                # Verwende den integrierten Handler
                if hasattr(self.ui_modernizer, '_handle_add_customer'):
                    return self.ui_modernizer._handle_add_customer()
            
            # Fallback
            return self._create_fallback_add_customer()
            
        except Exception as e:
            self.error_monitor.handle_error(e, "Create Customer", "error")
    
    def edit_customer(self):
        """Edit customer - shows customer selection first."""
        try:
            # Get list of customers
            customers = self.kunden_manager.alle_kunden()
            
            if not customers:
                messagebox.showinfo("Keine Kunden", "Keine Kunden vorhanden. Erstellen Sie zuerst einen Kunden.")
                return
            ...
            for customer in customers:
                btn = ctk.CTkButton(
                    main_frame,
                    text=customer,
                    command=lambda c=customer: self._edit_selected_customer(c, dialog),
                    width=350,
                    height=40
                )
                btn.pack(pady=5)
    ...
    def show_customer_list(self):
        """Shows the customer list - redirects to ModernCustomerGUI."""
        print("[DEBUG] show_customer_list called - redirecting to ModernCustomerGUI")
        try:
            # Umleitung zur modernen GUI anstatt der alten messagebox
            self.show_customer_menu()
        except Exception as e:
            print(f"[DEBUG] Error redirecting to ModernCustomerGUI: {e}")
            # Fallback zur alten Anzeige nur wenn die neue GUI komplett fehlschlägt
            try:
                customers = self.kunden_manager.alle_kunden()
                ...
                self.error_monitor.handle_error(fallback_error, "Show Customer List", "error")
