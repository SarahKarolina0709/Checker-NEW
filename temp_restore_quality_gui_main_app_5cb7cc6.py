...
try:
    ...
    print("Aggressive Anti-Dark-Mode nicht verf??gbar - verwende Fallback")
...
class ProfessionelleUebersetzungsqualitaetsApp:
    ...
    def _initialize_localization(self):
        ...
        try:
            self._i18n_map = {
                ...
                "Translation Quality Framework - Professional": "??bersetzungsqualit??ts-Framework - Professionell",
                # Sections / Headers
                "File Upload & Management": "Datei-Upload & Verwaltung",
                "File Upload": "Datei-Upload",
                "Analysis Configuration": "Analyse-Konfiguration",
                "Quality Criteria": "Qualit??tskriterien",
    ...
    def _initialize_design_system(self):
        ...
        try:
            # Import design_system.py f??r zentrale Farb-Verwaltung
            from design_system import DesignSystem
            design_sys = DesignSystem()
            return design_sys.get_full_system()
        except ImportError:
            # Fallback: Nutze ui_theme f??r zentrale Farben
            try:
    ...
    def _create_professional_feature_cards(self, parent):
        ...
        try:
            ...
            features_title = ctk.CTkLabel(
                parent,
                text="Hauptfunktionen & F??higkeiten",
        ...
        try:
            ...
            if current_projects:
                for i, project in enumerate(current_projects[:3]):  # Zeige nur die letzten 3
                    ...
                    # Projekt-Indikator (Text-basiert f??r NO ICONS POLICY)
    ...
    def _open_project_folder(self, project):
        ...
        try:
            ...
            self.logger.error(f"??? Fehler beim ??ffnen des Projekt-Ordners: {e}")
    ...
    def _open_projects_folder(self):
        ...
        try:
            ...
            self.logger.error(f"??? Fehler beim ??ffnen des Projekte-Ordners: {e}")
            self.show_toast("Fehler beim ??ffnen des Ordners", "error")
    ...
    def _refresh_project_navigation(self):
        ...
        try:
            # TODO: Implementation f??r dynamische UI-Aktualisierung
            # F??r jetzt einfaches Logging
    ...
    def manual_migration_dialog(self):
        ...
        try:
            ...
            # Frage ob Migration durchgef??hrt werden soll
            result = messagebox.askyesno(
                ...
                "Original-Dateien bleiben unver??ndert."
            )
            ...
            if result:
                ...
                if legacy_files:
                    ...
                    messagebox.showinfo(
                        ...
                        "Es wurden keine Dateien gefunden die migriert werden m??ssen."
                    )
            
        except Exception as e:
            self.logger.error(f"??? Fehler beim manuellen Migrations-Dialog: {e}")
            messagebox.showerror("Fehler", "Fehler bei der Migration. Siehe Log f??r Details.")
    ...
    def _create_analysis_dashboard(self):
        ...
        try:
            ...
            analysis_metrics = [
                ...
                ("Analysezeit", "2.3s", "primary", "Verarbeitungszeit f??r die Analyse")
            ]
    ...
    def _show_analysis_placeholder(self, parent):
        ...
        try:
            ...
            placeholder_label = ctk.CTkLabel(
                placeholder_frame,
                text="???? Bereit f??r die Analyse\n\nLaden Sie Ihre Ausgangs- und ??bersetzungsdateien hoch, um eine umfassende Qualit??tspr??fung zu starten.\n\nDas System erkennt automatisch Sprachen, analysiert die ??bersetzungsqualit??t\nund liefert detaillierte Verbesserungsvorschl??ge.",
                font=ctk.CTkFont(*self.get_typography('body_bold')),
                text_color=self.get_color('text_primary'),
                justify="center"
            )
    ...
    def _offer_open_project_folder(self, customer_name):
        ...
        try:
            ...
            self.logger.error(f"??? Fehler beim Anbieten des Ordner-??ffnens: {e}")
    ...
    def copy_files_to_project_structure(self, files, customer_name, project_date=None):
        ...
        try:
            ...
            if total_copied > 0:
                ...
                # Projektpfad f??r Benutzer loggen
    ...
    def _upload_source_files(self):
        ...
        try:
            ...
            files = filedialog.askopenfilenames(
                title="Ausgangstexte f??r Qualit??tsanalyse ausw??hlen",
                filetypes=file_types
            )
            
            if files:
                # ???? STRUKTUR-INTEGRATION: Kunde optional ausw??hlen f??r Projektstruktur
                ...
                if new_files:
                    if customer_name:
                        ...
                        # ???? Optional: Projektordner ??ffnen anbieten
                        self._offer_open_project_folder(customer_name)
                    else:
                        # Ohne Kunden: Dateien direkt zur Upload-Liste hinzuf??gen
                        self.uploaded_files['source'].extend(new_files)
                        self.show_toast(f"??? {len(new_files)} Datei(en) hinzugef??gt (ohne Projektstruktur)", "success")
                    
                    self._update_file_counter()
                    self._refresh_file_list_display()
                    self._show_enhanced_upload_results("source", new_files)
                    
                    # Smart File Pairing ausf??hren
    ...
    def _upload_batch_files(self):
        ...
        try:
            ...
            files = filedialog.askopenfilenames(
                title="Dateien f??r Stapel-Upload und Analyse ausw??hlen",
                filetypes=file_types
            )
            ...
            if files:
                ...
                for i, file_path in enumerate(files):
                    try:
                        ...
                        is_translation = any(keyword in filename.lower() for keyword in [
                            ...
                            '??bersetzung', '??bersetzt', 'ziel', 'target_language'
                        ])
                ...
                # Smart File Pairing ausf??hren nach Upload
    ...
    def _update_file_counter(self):
        ...
        try:
            ...
            if hasattr(self, 'analyze_button'):
                if source_count > 0 and translation_count > 0:
                    self.analyze_button.configure(
                        state="normal",
                        fg_color=self.get_color('primary'),
                        text=self._t("Qualit??tsanalyse starten")
                    )
                else:
                    self.analyze_button.configure(
                        state="disabled",
            ...
            # MANUAL PAIRING UI - Zeige Manual Pairing Option wenn n??tig
            self._check_and_show_manual_pairing_option()
    ...
    def _smart_file_pairing(self):
        ...
        try:
            ...
            if not source_files or not translation_files:
                return  # Keine Paarung m??glich ohne beide Dateitypen
            
            # Erstelle Paarung basierend auf Dateinamen-??hnlichkeit
            pairs = []
            unmatched_source = []
            ...
            for source_file in source_files:
                ...
                for trans_file in unmatched_translation:
                    ...
                    if similarity > best_score and similarity > 0.6:  # 60% ??hnlichkeit minimum
                ...
                if best_match:
                    ...
                    # Entferne gematched translation aus verf??gbaren
                    unmatched_translation.remove(best_match)
                else:
                    unmatched_source.append(source_file)
            
            # Speichere Paarung f??r sp??tere Nutzung
    ...
    def _normalize_filename(self, filepath):
        """???? NORMALIZE FILENAME - Bereinige Dateiname f??r Vergleich"""
        try:
            ...
            remove_patterns = [
                ...
                '_original', '_orig', '_src', '_??bersetzung', '_??bersetzt',
    ...
    def _calculate_filename_similarity(self, name1, name2):
        """???? CALCULATE FILENAME SIMILARITY - Berechne ??hnlichkeit zwischen Dateinamen"""
        try:
            if name1 == name2:
                return 1.0
                
            # L??ngste gemeinsame Teilsequenz
            from difflib import SequenceMatcher
            similarity = SequenceMatcher(None, name1, name2).ratio()
            
            # Bonus f??r exakte Kern-??bereinstimmung
            if name1 in name2 or name2 in name1:
                similarity += 0.2
                
            # Bonus f??r Wort-??bereinstimmungen
    ...
    def _display_file_pairing_results(self, pairs, unmatched_source, unmatched_translation):
        ...
        try:
            ...
            if pairs:
                ...
                for pair in pairs:
                    logger.info(f"???? Pair: {pair['source_name']} ??? {pair['translation_name']} (??hnlichkeit: {pair['similarity']:.1%})")
            else:
                self.show_toast("Keine automatische Dateipaarung m??glich - Namen zu unterschiedlich", "warning", duration=3000)
            ...
            if unmatched_translation:
                status_parts.append(f"{len(unmatched_translation)} ungepaarte ??bersetzungen")
            ...
            if source_count > 0 and translation_count > 0:
                ...
                if unmatched_total > 0 or source_count != translation_count:
                    self.root.after(2000, lambda: self.show_toast(
                        "Manuelles Pairing verf??gbar - Klicke auf 'Dateipaarung anpassen'", 
                        "info", 
                        duration=5000
                    ))
    ...
    def _create_pair_display_item(self, parent, pair, index):
        ...
        try:
            ...
            arrow_label = ctk.CTkLabel(
                arrow_frame,
                text=f"??? ??hnlichkeit: {pair['similarity']:.1%}",
                font=ctk.CTkFont(*self.get_typography('caption')),
                text_color=self.get_color('text_secondary')
            )
            arrow_label.pack()
        ...
        try:
            ...
            if file_type == "source":
                ...
                if available_translations:
                    dropdown_values = ["W??hle ??bersetzung..."] + [os.path.basename(f) for f in available_translations]
                    ...
                    pair_dropdown.set("W??hle ??bersetzung...")
                else:
                    # Keine verf??gbaren ??bersetzungen
                    no_trans_label = ctk.CTkLabel(
                        content_frame,
                        text="Keine ungepaarten ??bersetzungen verf??gbar",
                        font=ctk.CTkFont(*self.get_typography('caption')),
                        text_color=self.get_color('text_secondary')
                    )
                ...
                if available_sources:
                    dropdown_values = ["W??hle Ausgangstext..."] + [os.path.basename(f) for f in available_sources]
                    ...
                    pair_dropdown.pack(fill="x", pady=(5, 0))
                    pair_dropdown.set("W??hle Ausgangstext...")
                else:
                    # Keine verf??gbaren Quellen
                    no_source_label = ctk.CTkLabel(
                        content_frame,
                        text="Keine ungepaarten Ausgangstexte verf??gbar",
                        font=ctk.CTkFont(*self.get_typography('caption')),
                        text_color=self.get_color('text_secondary')
                    )
    ...
    def _manual_pair_files(self, source_file, selected_translation, file_type):
        ...
        try:
            if selected_translation == "W??hle ??bersetzung...":
                return
            ...
            if not translation_file:
                self.show_toast("??bersetzungsdatei nicht gefunden", "error")
                return
    ...
    def _manual_pair_files_translation(self, translation_file, selected_source, file_type):
        ...
        try:
            if selected_source == "W??hle Ausgangstext...":
                return
    ...
    def _unpair_files(self, pair_index):
        ...
        try:
            ...
            self.show_toast(f"Paarung aufgel??st: {pair['source_name']} ??? {pair['translation_name']}", "info")
    ...
    def _clear_all_files(self):
        ...
        try:
            ...
            print(f"Fehler beim L??schen der Dateien: {e}")
    ...
    def _upload_translation_files(self):
        """???? ERWEITERT: ??bersetzungsdatei-Upload mit automatischer Projektstruktur-Integration"""
        try:
            # Smart file type detection with extended support
            file_types = [
                ("All Supported", "*.pdf;*.docx;*.txt;*.doc;*.rtf;*.odt"),
            ...
            files = filedialog.askopenfilenames(
                title="??bersetzungsdateien f??r Qualit??tsanalyse ausw??hlen",
                filetypes=file_types
            )
            ...
            if files:
                ...
                if new_files:
                    if customer_name:
                        ...
                        self.show_toast(f"??? {len(new_files)} ??bersetzung(en) hinzugef??gt (ohne Projektstruktur)", "success")
                    ...
                    status_msg = f"{len(new_files)} ??bersetzungsdateien hinzugef??gt"
                    if duplicate_count > 0:
                        status_msg += f" ({duplicate_count} Duplikate ignoriert)"
                    ...
                    self.update_status(status_msg)
                    self.show_toast(f"{len(new_files)} ??bersetzungsdateien erfolgreich hochgeladen", "success")
            ...
            error_msg = f"Fehler beim Hochladen der ??bersetzungsdateien: {e}"
            self.update_status(error_msg)
