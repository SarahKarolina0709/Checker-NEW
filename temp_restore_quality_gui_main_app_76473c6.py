...
# -*- coding: utf-8 -*-
"""
Professionelle ??bersetzungsqualit??ts-GUI - Hauptanwendung
Erweiterte 2-Panel-Ansicht mit umfassenden Qualit??tsanalyse-Funktionen
"""
...
import logging
import time
import copy  # Undo/Redo ben??tigt tiefe Kopien von Pairing-State
from pathlib import Path
import tkinter as tk
import customtkinter as ctk
try:  # Zentrales UIHelper-System f??r konsistente Button-Styles (wie Welcome Screen)
    from src.utils.ui_helpers import UIHelpers
except Exception:
    UIHelpers = None
from tkinter import filedialog, messagebox
import time  # ??? F??r Migrations-Timestamp (Auto-Migration Marker)
...
if not hasattr(ctk, '_quality_anti_dark_applied'):
    try:
        ...
        logger.warning("Aggressive Anti-Dark-Mode nicht verf??gbar - verwende Fallback")
...
class ProfessionelleUebersetzungsqualitaetsApp:
    """Professionelle ??bersetzungsqualit??ts-GUI mit erweiterten Funktionen"""
    ...
    def __init__(self):  # Wiederhergestellt aus Backup & an neue Struktur angepasst
        ...
        Minimal-invasiv: Keine bestehende Logik entfernt, nur fehlende Initialisierung erg??nzt.
        """
        try:  # Gesamter Initialisierungsblock robust gekapselt
            ...
            self.STANDARD_PROJECT_STRUCTURE = [
                "01_Ausgangstext", "02_Angebot", "03_Pr??fung", "04_Finalisierung"
            ]
            ...
            # -------------------- SYSTEM-KOMPONENTEN (urspr??nglich vorhanden) --------------------
            ...
            # Optionale Infrastruktur (nur falls verf??gbar) ??? verhindert AttributeError in Callbacks
            try:
            ...
            # Einheitliche Farb-API fr??h bereitstellen
            try:
            ...
            try:
                ...
                self.logger.warning(f"Projekt-Migration ??bersprungen: {e}")

        except Exception as e:  # letzter Fallback ??? niemals Absturz nach au??en
            try:
    ...
    _CONTEXT_DEFAULT_MAP = {
        # Files
        "files.counter.update": "Dateiz??hler konnte nicht aktualisiert werden",
        ...
        # Upload
        "upload.translation": "??bersetzungsdateien Upload fehlgeschlagen",
        ...
        "pairing.manual.interface": "Manuelle Pairing Oberfl??che Fehler",
        "pairing.manual.populate": "Manuelle Pairing Bef??llung Fehler",
    ...
    def _apply_localization_safe(self):
        """Wendet sichere Lokalisierung auf prim??re UI-Elemente an (Fenstertitel)."""
        try:
    ...
    # ========================= DESIGN SYSTEM INITIALIZER (aus Backup ??bernommen) =========================
    def _initialize_design_system(self):
        """DESIGN SYSTEM - Zentrale Farb-/Spacing-/Typografie-Verwaltung (Instruction-konform).

        ??bernommen aus Backup-Version; nur Kommentar leicht angepasst. Stellt vollst??ndiges
        System bereit oder liefert einen ui_theme Fallback. Verwendet KEINE hartcodierten Hex-Farben
        au??erhalb der Fallback-Ebene (Instruction: Hex nur in finalem Notfall erlaubt).
        """
        try:
            from design_system import DesignSystem  # Lokaler Import f??r robustes Lazy-Loading
            design_sys = DesignSystem()
            return design_sys.get_full_system()
        except ImportError:
            # Fallback auf ui_theme (Legacy) ??? nur falls design_system Modul nicht verf??gbar
            try:
                ...
                return {
                    'colors': {
                        ...
                        # Vereinheitlichung: Sekund??re Buttons jetzt auch Blau
    ...
    def _handle_error(self, e: Exception, context: str = "", user_message: str | None = None, level: str = "error", toast: bool = True, event_name: str | None = None, **extra):
        ...
        Parameters:
            ...
            event_name: Optionales Event f??r event_bus
            extra: Zus??tzliche Felder f??r Logging / Event
        """
        try:
            # 1) Logging (error-level mit exception() f??r direkten Trace)
            if level == "error":
                try:
            ...
            # 3) Automatische Standard-Meldung falls keine user_message ??bergeben
    ...
    def _initialize_localization_map(self):
        self._i18n_map = {
            ...
            "Upload Translations": "??bersetzungen laden",
            ...
            "No analysis function available": "Keine Analyse-Funktion verf??gbar",
            "All": "Alle",
            "Critical": "Kritisch",
            "Major": "Schwerwiegend",
            # Error / Status (added)
            ...
            "Hauptoberfl??che Fehler": "Hauptoberfl??che Fehler",
            "Inhaltsbereich konnte nicht erstellt werden": "Inhaltsbereich konnte nicht erstellt werden",
            "Qualit??tskriterien konnten nicht erstellt werden": "Qualit??tskriterien konnten nicht erstellt werden",
            ...
            "Projekt-Ordner konnte nicht ge??ffnet werden": "Projekt-Ordner konnte nicht ge??ffnet werden",
            "Projekte-Verzeichnis konnte nicht ge??ffnet werden": "Projekte-Verzeichnis konnte nicht ge??ffnet werden",
            "Neues Projekt konnte nicht erstellt werden": "Neues Projekt konnte nicht erstellt werden",
            "Projektstruktur ung??ltig": "Projektstruktur ung??ltig",
            ...
            "Dateiz??hler konnte nicht aktualisiert werden": "Dateiz??hler konnte nicht aktualisiert werden",
            ...
            "Manuelle Pairing Oberfl??che Fehler": "Manuelle Pairing Oberfl??che Fehler",
            "Manuelle Pairing Bef??llung Fehler": "Manuelle Pairing Bef??llung Fehler",
            ...
            "Dateien L??schen Fehler": "Dateien L??schen Fehler",
            "Dateiliste Refresh Fehler": "Dateiliste Refresh Fehler",
            "??bersetzungsdateien Upload fehlgeschlagen": "??bersetzungsdateien Upload fehlgeschlagen",
            ...
            "Export Modul nicht verf??gbar": "Export Modul nicht verf??gbar",
            ...
            # Status / Labels
            "Translation Quality Framework": "??bersetzungsqualit??ts-Framework",
            "Translation Quality Framework - Professional": "??bersetzungsqualit??ts-Framework ??? Professional",
            "Professional Quality Analysis": "Professionelle Qualit??tsanalyse",
            # Neue UI Sektionen / Zus??tzliche Strings
            "File Upload": "Datei-Upload",
            "File Upload & Management": "Datei-Upload & Verwaltung",
            "Application Settings & Preferences": "Anwendungseinstellungen & Pr??ferenzen",
            "Detailed Quality Analysis Report": "Detaillierter Qualit??tsanalyse-Bericht",
            "Additional Actions": "Weitere Aktionen",
            "Translation Quality Analysis Results": "??bersetzungsqualit??ts-Analyseergebnisse",
            "System Status": "Systemstatus",
            "Overall Quality": "Gesamtqualit??t",
            "Issues Detected": "Gefundene Probleme",
            "Recommendations": "Empfehlungen",
            "Excellent translation quality": "Hervorragende ??bersetzungsqualit??t",
            "Minor issues found": "Kleinere Auff??lligkeiten gefunden",
            "Improvement suggestions": "Verbesserungsvorschl??ge",
            ...
            "Ready": "Bereit",
            "Key Features & Capabilities": "Hauptfunktionen & F??higkeiten",
            "Advanced Analysis Engine": "Intelligente Analyse-Engine",
            "Professional Reporting": "Detaillierte Berichte",
            "Files: 0 source, 0 translations": "Dateien: 0 Ausgangstexte, 0 ??bersetzungen",
            "Supported formats: PDF, DOCX, TXT, DOC, RTF, ODT ??? Drag and drop supported": "Unterst??tzte Formate: PDF, DOCX, TXT, DOC, RTF, ODT ??? Drag & Drop unterst??tzt",
            ...
            "Could not open full findings view": "Vollst??ndige Ergebnisansicht konnte nicht ge??ffnet werden",
            "Copied to clipboard": "In Zwischenablage kopiert",
            "Export created": "Export erstellt",
            "No findings available": "Keine Ergebnisse verf??gbar",
            # Pairing
            "Translation file not found": "??bersetzungsdatei nicht gefunden",
            "Source file not found": "Ausgangsdatei nicht gefunden",
            "Pair created": "Paar erstellt",
            "Pair removed": "Paarung aufgel??st",
            "Unpair": "Trennen",
            "Source": "Ausgangstext",
            ...
            "Manual pairing available - Click 'Adjust file pairing'": "Manuelles Pairing verf??gbar - Klicke auf 'Dateipaarung anpassen'",
            "Automatic file pairing not possible - names too different": "Keine automatische Dateipaarung m??glich - Namen zu unterschiedlich",
            "file pair(s)": "Dateipaar(e)",
            "unpaired file(s)": "ungepaarte Datei(en)",
            "unpaired source file(s)": "ungepaarte Quelldateien",
            "unpaired translation(s)": "ungepaarte ??bersetzungen",
            "Select translation...": "W??hle ??bersetzung...",
            "Select source...": "W??hle Ausgangstext...",
            ...
            "No unmatched translations available": "Keine ungepaarten ??bersetzungen verf??gbar",
            "No unmatched sources available": "Keine ungepaarten Ausgangstexte verf??gbar",
            # Drag & Drop Pairing
    ...
    def _basic_get_color(self, color_name: str, fallback: str = '#FFFFFF'):
        """Sicherer Farbzugriff ??? ausschliesslich ??ber DesignSystem + Mini-Fallback.
        ...
        try:
            ...
            # Anti-Dark-Mode Schutz anwenden falls verf??gbar
            try:
    ...
    def ui(self, fn, *a, **k):
        """Thread-sicheres Dispatch von UI-Operationen ??ber root.after."""
        try:
    ...
    def get_typography(self, typography_name):
        ...
        try:
            ...
            # GOVERNANCE LOCK: Eingeschr??nkte Legacy Tokens (bereinigt ??? keine aktiv genutzten Tokens blockieren)
            legacy_block = {
            ...
            optimized_typography = {
                ...
                # ???? Unified Ziel-Tokens (nur interne Nutzung ??? bevorzugt externe Aufrufe ??ber caption/label/body/... Mapping)
    ...
    def _setup_application(self):
        ...
        try:
            self.root = ctk.CTk()
            self.root.title("??bersetzungsqualit??ts-Framework - Professional Edition")
                        ...
                        try:
                            ...
                            for attr in [
                                ...
                                try:
                                    ...
                                    pass
                            # DesignSystem Caches leeren falls verf??gbar
                            try:
    ...
    def _generate_dynamic_report(self):
        ...
        - Nicht destruktiv: Legt/??berschreibt nur 'Bericht_Dynamisch.html' im Modulverzeichnis
        ...
        try:
            ...
            # Sicher serialisieren (ensure_ascii=False f??r Umlaute)
            try:
...
<html lang=\"de\">\n<head>\n<meta charset=\"UTF-8\" />\n<title>Dynamischer Qualit??tsbericht</title>\n<meta name=\"viewport\" content=\"width=device-width,initial-scale=1\" />\n<style>\n body{{font-family:Segoe UI,Arial,sans-serif;background:{color_surface};margin:24px;color:{color_text};}}\n h1{{font-size:24px;margin:0 0 8px;font-weight:600;}}\n h2{{font-size:18px;margin:32px 0 12px;font-weight:600;}}\n .meta, .summary{{background:{color_panel};border:1px solid {color_border};border-radius:8px;padding:16px;margin-bottom:20px;}}\n code,pre{{font-family:Consolas,monospace;font-size:12px;}}\n .counts span{{display:inline-block;margin:4px 8px 4px 0;padding:4px 8px;border:1px solid {color_badge_border};border-radius:6px;background:{color_badge_bg};}}\n .raw-container{{border:1px solid {color_border};border-radius:8px;padding:16px;white-space:pre;overflow:auto;max-height:480px;background:{color_surface};}}\n .footer{{margin-top:40px;font-size:12px;color:{color_footer};}}\n button{{background:{color_button};color:{color_white};border:none;padding:8px 14px;border-radius:6px;cursor:pointer;font-size:14px;font-weight:600;}}\n button:hover{{background:{color_button_hover};}}\n .toolbar{{margin:0 0 16px;}}\n .filter-box{{border:1px solid {color_filter_border};border-radius:6px;padding:8px;margin:0 0 16px;}}\n input[type=text]{{padding:6px 8px;border:1px solid {color_filter_border};border-radius:4px;width:260px;}}\n table{{border-collapse:collapse;width:100%;margin-top:12px;}}\n th,td{{border:1px solid {color_border};padding:6px 8px;font-size:12px;text-align:left;vertical-align:top;}}\n th{{background:{color_table_header};font-weight:600;}}\n .hidden{{display:none;}}\n</style>\n</head>\n<body>\n<h1>Qualit??tsbericht (Dynamisch)</h1>\n<div class=\"meta\">\n  <strong>Erstellt:</strong> {meta_generated_at}<br/>\n  <strong>Quelle:</strong> {meta_source}<br/>\n</div>\n<div class=\"summary\">\n  <h2>Struktur Zusammenfassung</h2>\n  <div class=\"counts\">{counts_html}</div>\n  <p>Gefilterte Ansicht: Verwende Suchfeld f??r einfache Textfilterung (client-side). Rohdaten unten vollst??ndig.</p>\n</div>\n<div class=\"filter-box\">\n  <input id=\"filterInput\" type=\"text\" placeholder=\"Suchen...\" oninput=\"applyFilter()\" />\n  <button onclick=\"resetFilter()\">Zur??cksetzen</button>\n</div>\n<div id=\"dynamicTables\"></div>\n<h2>Rohdaten JSON</h2>\n<div class=\"raw-container\" id=\"rawJson\"></div>\n<div class=\"footer\">Automatisch generiert ??? tempor??re Datei (Bericht_Dynamisch.html).</div>\n<script>\nconst payload = {json_blob};\nconst reportData = payload.reportData || {{}};\nfunction esc(s){{return String(s).replace(/[&<>\\\"']/g, c=>({{'&':'&amp;','<':'&lt;','>':'&gt;','\\\"':'&quot;','\'':'&#39;'}})[c]);}}\nfunction buildTables(){{\n  const container=document.getElementById('dynamicTables');\n  container.innerHTML='';\n  const keys=Object.keys(reportData);\n  if(!keys.length){{container.innerHTML='<em>Keine Daten</em>';return;}}\n  keys.forEach(k=>{{\n    const section=document.createElement('div');\n    section.className='data-section';\n    const val=reportData[k];\n    let html='';\n    if(Array.isArray(val) && val.length && typeof val[0]==='object'){{\n       const cols=Array.from(new Set(val.flatMap(o=>Object.keys(o))));\n       html += `<h2>${{esc(k)}}</h2><table><thead><tr>${{cols.map(c=>`<th>${esc(c)}</th>`).join('')}} </tr></thead><tbody>`;\n       val.forEach(row=>{{html+='<tr>'+cols.map(c=>`<td>${esc(row[c]!==undefined?row[c]:'')}</td>`).join('')+'</tr>';}});\n       html+='</tbody></table>';\n    }} else if (Array.isArray(val)) {{\n       html += `<h2>${{esc(k)}}</h2><div>${{val.map(i=>`<div>- ${esc(i)}</div>`).join('')}} </div>`;\n    }} else if (val && typeof val==='object') {{\n       html += `<h2>${{esc(k)}}</h2><pre>${{esc(JSON.stringify(val,null,2))}}</pre>`;\n    }} else {{\n       html += `<h2>${{esc(k)}}</h2><div>${{esc(val)}}</div>`;\n    }}\n    section.innerHTML=html;\n    container.appendChild(section);\n  }});\n}}\nfunction applyFilter(){{\n  const q=document.getElementById('filterInput').value.toLowerCase();\n  document.querySelectorAll('.data-section').forEach(sec=>{{\n    sec.classList.remove('hidden');\n    if(q){{ if(sec.textContent.toLowerCase().indexOf(q)===-1) sec.classList.add('hidden'); }}\n  }});\n}}\nfunction resetFilter(){{document.getElementById('filterInput').value='';applyFilter();}}\nfunction init(){{\n  buildTables();\n  document.getElementById('rawJson').textContent=JSON.stringify(payload,null,2);\n}}\ninit();\n</script>\n</body>\n</html>""".format(
    ...
    def _initialize_infrastructure_async(self):
        """Starte Infrastruktur Lazy im Hintergrund f??r minimale Startup-Latenz."""
        try:
        ...
        try:
            ...
            try:
                if not hasattr(self, 'theme_guard'):
                    ...
                    # Wrapper f??r vorhandene get_color Methode nur hinzuf??gen falls noch nicht dekoriert
    ...
    def submit_background_task(self, func: Callable, *args, **kwargs) -> bool:
        """??ffentliche Helper-API f??r zuk??nftige Features zur Nutzung des WorkerPools.
        R??ckgabe: True wenn angenommen, sonst False (f??llt ger??uschlos zur??ck)."""
        try:
            if self.worker_pool:
    ...
    def _analyze_with_plugins(self, context: dict) -> list:
        """F??hre geladene Plugin-Regeln sicher aus und liefere RuleResult Objekte.
        ...
        results = []  # Rohwerte (Kompatibilit??t beibehalten)
        stats = {
            "executed": 0,      # Versuchte Ausf??hrungen (inkl. Fehler/Timeout)
            "timeouts": 0,
            "errors": 0,
        ...
        try:
            ...
            # Konfigurierbares Logging-Level f??r Timeouts
            ...
            for rule_cls in loaded:
                # Gesamt-Budget pr??fen
                if (_t.time() - start_overall) > overall_budget:
                    try:
                        self.logger.warning("Plugin-Analyse abgebrochen: Budget ??berschritten (%.2fs > %.2fs)", (_t.time() - start_overall), overall_budget)
                    except Exception:
                        pass
                    break
                ...
                try:
                    with _f.ThreadPoolExecutor(max_workers=1) as ex:
                        ...
                        try:
                            ...
                            if val is not None:
                                # Struktur beibehalten (Rohwert); optional k??nnte normalisiert werden
                                results.append(val)
                    ...
                    # Executor Failure ??? als Fehler z??hlen
                    duration = _t.time() - rule_start
                    error = True
                    stats["errors"] += 1
                    try:
            ...
            try:
                if getattr(self, 'event_bus', None):
                    self.event_bus.publish("plugins.analysis.completed", {
                        "count": len(results),
                        # Mini-Patch A: Regelnamen aus per_rule Stats (robust, unabh??ngig vom Resultat-Typ)
    ...
    def _create_button(self, parent, text: str, command=None, kind: str = "primary", **overrides):
        ...
        Zus??tzliche optionale Overrides:
          size: 'sm' | 'md' | 'lg' (mappt auf heights.button_sm/md/lg)
          width/height: explizit m??glich (??berschreibt size)
        Alle Farben / Ma??e kommen aus dem Design-System (keine Hardcodes).
        """
        try:
            size = overrides.pop('size', None)  # optionaler semantischer Gr????en-Parameter
            ...
            # H??hen-Tokens (Fallbacks sichern)
            ...
            # Welcome-Screen Stil: bevorzugt semantische Button-Font falls verf??gbar
            try:
            ...
            # Basis-H??he prim??r ??ber size oder Token
            ...
            base_cfg = dict(
                ...
                height=int(ds_height),  # Outline justiert sp??ter ggf.
                border_width=0,
                anchor="center",
            )

            # Outline Varianten wie Welcome Screen (wei??er Hintergrund, farbiger Text, feine Border)
            if kind in {"outline-primary", "outline", "outline-danger", "outline-warning"}:
                # Welcome Screen Outline Stil (wei??, feine Border, farbiger Text)
                ...
                # H??he konsistent zu Welcome Screen bevorzugt heights.button_md
                outline_height = h_md
                base_cfg.update(
                    fg_color=self.get_color('white'),
                    hover_color=self.get_color('surface_hover'),  # dezente Hover-Fl??che
                    text_color=txt_col,
                    border_width=1,
            ...
            # Delegiere optional an UIHelpers f??r konsistentes Enabled/Disabled Styling
            try:
    ...
    def _setup_left_panel(self):
        ...
        try:
            ...
            header_frame = ctk.CTkFrame(
                self.left_panel, 
                fg_color=self.get_color('primary'),  # Vereinheitlicht: gleicher Prim??r-Blau-Ton
            ...
            subtitle_label = ctk.CTkLabel(
                header_content,
                text="Professionelle Qualit??tskontrolle",
    ...
    def _create_minimal_left_panel(self):
        ...
        try:
            ...
            # Basic upload button
            # Vereinheitlicht ??ber zentrale Button-Factory
    ...
    def _create_upload_section(self, parent):
        ...
        try:
            ...
            info_label = ctk.CTkLabel(
                info_frame,
                text="Supported formats: PDF, DOCX, TXT, DOC, RTF, ODT ??? Drag & Drop unterst??tzt",
                font=ctk.CTkFont(*self.get_typography('caption')),
                text_color=self.get_color('text_secondary'),
                wraplength=540,
                justify="left",
    ...
    def _create_basic_upload_section(self, parent):
        ...
        try:
            ...
            translation_btn = self._create_button(
                ...
                command=self._upload_translation_files,
                # Konsistenz: Outline-Variante f??r sekund??re Aktion
        ...
        try:
            ...
            for item, _ in widgets:
                try:
                    ...
                    pass
            # Spalten IMMER definieren (R??cksetzen im Wide-Fall inklusive)
            for i in (0,1,2):
    ...
    def _show_settings_dashboard(self):
        ...
        try:
            ...
            if not hasattr(self, 'LANG_DISPLAY2ISO'):
                ...
                # Erg??nze evtl. fehlende Gro??-/Kleinschreibungs-Varianten
            ...
            # Notification / Toast Settings (separate card - keine bestehenden Keys ver??ndern)
            try:
    ...
    # Alte _create_professional_settings_section entfernt (Funktionalit??t in Dashboard integriert)
    ...
    def _create_main_interface(self):
        ...
        try:
            ...
            self._handle_error(
                ...
                user_message=self._t("Hauptoberfl??che Fehler") if hasattr(self, '_t') else "Main Interface Fehler"
            )
    ...
    def _create_quality_criteria_section(self, parent):
        ...
        try:
            ...
            header_label = ctk.CTkLabel(
                header_content,
                text="3. Qualit??tskriterien",  # Unified German title
                font=ctk.CTkFont(*self.get_typography('subtitle_unified')),
            ...
            self._handle_error(
                ...
                user_message=self._t("Qualit??tskriterien konnten nicht erstellt werden") if hasattr(self, '_t') else "Quality Criteria Fehler"
            )
    ...
    def _create_actions_section(self, parent):
        ...
        try:
            ...
            # Vereinheitlichter Fallback laut Vorgabe: keine UI-Teil-Erstellung hier, direkte R??ckfallebene
            self._handle_error(
                e,
    ...
    def _setup_right_panel(self):
        ...
        try:
            ...
            subtitle_label = ctk.CTkLabel(
                subtitle_frame,
                text="Professionelles Qualit??ts-Dashboard",
                font=ctk.CTkFont(*self.get_typography('caption')),
                text_color=self.get_color('gray_300', self.get_color('white'))
            )
    ...
    def _setup_basic_right_panel(self):
        ...
        try:
            ...
            header_frame = ctk.CTkFrame(
                self.right_panel,
                # Vereinheitlicht: gleicher Prim??r-Farbton wie linkes Panel
                fg_color=self.get_color('primary'),
                height=60
            )
            header_frame.pack(fill="x", padx=0, pady=0)
    ...
    def _create_professional_metrics_dashboard(self, parent):
        ...
        try:
            # Vereinheitlichter Header-Stil (gleiche St??rke wie linke Panel-Boxen)
            header_frame = ctk.CTkFrame(
                parent,
            ...
            metrics_title = ctk.CTkLabel(
                header_inner,
                text="System??bersicht",
                font=ctk.CTkFont(*self.get_typography('heading')),
                text_color=self.get_color('white')
            )
            metrics_title.pack(side="left")
            
            # Status Indikator - Einheitlich gr??n
            ...
            # Mehr vertikaler Abstand zum Header f??r bessere Luft
            ...
            for i, (value, title, color) in enumerate(metrics):
                ...
                # Gr????erer horizontaler Abstand zwischen den Kacheln f??r eleganteres Layout
                metric_card.grid(row=0, column=i, sticky="ew", padx=12, pady=8)
                
                # OPTIMIERT: Konsistente Typografie - gleiche Gr????e f??r alle Werte
                value_label = ctk.CTkLabel(
                    metric_card,
                    text=value,
    ...
    def _create_feature_cards(self, parent):
        """PROFESSIONAL: Feature Cards (einheitliche Version, i18n-f??hig)."""
        try:
            # Hauptfunktionen & F??higkeiten Header
            # Harmonisierte Typografie: ??berschrift als heading_sm
            features_title = ctk.CTkLabel(
                parent,
                text=self._t("Hauptfunktionen & F??higkeiten"),
    ...
    def _open_project_folder(self, project):
        """???? Projekt-Ordner im Explorer ??ffnen"""
        try:
            import subprocess
            import sys
            ...
            if os.path.exists(project_path):
                ...
                self.show_toast(f"Projekt-Ordner ge??ffnet: {project['full_name']}", "success")
            else:
            ...
            self._handle_error(
                ...
                user_message=self._t("Projekt-Ordner konnte nicht ge??ffnet werden") if hasattr(self, '_t') else "Projekt-Ordner ??ffnen Fehler"
            )
            self.show_toast("Fehler beim ??ffnen des Ordners", "error")
    
    def _open_projects_folder(self):
        """???? Hauptprojekte-Ordner im Explorer ??ffnen (Checker_Projekte)"""
        try:
            import subprocess
            import sys
            ...
            self.show_toast("Checker-Projekte-Ordner ge??ffnet", "success")
            
        except Exception as e:
            self._handle_error(
                e,
                context="projects.open.base",
                user_message=self._t("Projekte-Verzeichnis konnte nicht ge??ffnet werden") if hasattr(self, '_t') else "Projekte-Verzeichnis Fehler"
            )
            self.show_toast("Fehler beim ??ffnen des Ordners", "error")
    
    def _create_new_project_dialog(self):
        """???? Dialog f??r neues Projekt erstellen"""
        try:
    ...
    def _validate_all_projects(self):
        ...
        try:
            ...
            if repaired_count > 0:
                ...
                self.show_toast("Alle Projektstrukturen sind vollst??ndig", "info")
            
        except Exception as e:
            self._handle_error(
                e,
                context="projects.structure.validate",
                user_message=self._t("Projektstruktur ung??ltig") if hasattr(self, '_t') else "Struktur Validierung Fehler"
            )
            self.show_toast("Fehler bei der Struktur-Validierung", "error")
    
    def _refresh_project_navigation(self):
        """???? Projekt-Navigation aktualisieren (vollst??ndiger Rebuild der Welcome-Ansicht)."""
        try:
            if hasattr(self, '_show_enhanced_welcome_output'):
                try:
                    self._show_enhanced_welcome_output()
    ...
    def _migrate_existing_projects_on_startup(self):
        ...
        try:
            # Migration nur beim ersten Start nach Update durchf??hren
            migration_marker = "migration_v1_completed.marker"
            
            if os.path.exists(migration_marker):
                return  # Migration bereits durchgef??hrt
            
            self.logger.info("???? Starte automatische Projekt-Migration...")
            
            # Bestehende Dateien und Ordner scannen (sp??ter deduplizieren)
    ...
    def _classify_legacy_file(self, file_path):
        """???? Klassifiziert Legacy-Dateien f??r Migration"""
        filename = os.path.basename(file_path).lower()
        
        # ??bersetzungs-Keywords
        translation_keywords = ['translation', 'trans', '??bersetzung', 'target', 'tgt', 'output', 'result']
        ...
        try:
            ...
            for file_info in legacy_files:
                try:
                    ...
                    # Datei kopieren (nicht verschieben f??r Sicherheit)
                    ...
                    # Einzeldatei-Migrationsfehler ohne Toast (Spam-Pr??vention)
                    self._handle_error(
                        e,
    ...
    def manual_migration_dialog(self):
        """???? Manueller Migrations-Dialog f??r Benutzer"""
        try:
            ...
            self._show_non_blocking_confirm(
                ...
                message=(
                    ...
                    self._t("Original-Dateien bleiben unver??ndert.") if hasattr(self, '_t') else "Original-Dateien bleiben unver??ndert."
                ),
    ...
    def _execute_manual_migration(self):
        """F??hrt die eigentliche Migration nach Best??tigung asynchron aus."""
        try:
            legacy_files = self._scan_for_legacy_files()
            if legacy_files:
                migrated_count = self._migrate_legacy_files(legacy_files)
                ...
                msg = self._t("Keine Dateien f??r Migration gefunden.") if hasattr(self, '_t') else "Keine Dateien f??r Migration gefunden."
    ...
    def _show_non_blocking_confirm(self, title: str, message: str, confirm_text: str, cancel_text: str, on_confirm, width: int = 520):
        """Generischer nicht-blockierender Best??tigungsdialog (Design-System konform)."""
        try:
            ...
            # ??? UX: Abbrechen links, Prim??raktion rechts
    ...
    def _center_window(self, window, width=None, height=None, anchor_root=True):
        """Zentriert ein Toplevel-Fenster robust. Rein kosmetisch; Fehler werden unterdr??ckt."""
        try:
    ...
    def update_file_count(self, count):
        ...
        try:
            ...
            if hasattr(self, 'file_count_badge'):
                # Kurzer Badge Text (engl. UI Basis); optional sp??ter lokalisierbar
                self.file_count_badge.configure(text=f"{count} files")
        except Exception as e:
            self._handle_error(
                e,
                context="files.counter.update",
                user_message=self._t("Dateiz??hler konnte nicht aktualisiert werden") if hasattr(self, '_t') else "File Counter Fehler",
                toast=False
            )
    ...
    def show_toast(self, message: str, type: str = "info", duration: int = 3000):
        ...
        try:
            ...
            # Erfolg & Warnung visuell wie Info (Primary), nur Systemstatus bleibt wirklich gr??n.
            original_type = type
    ...
    def _show_toast(self, message: str, type: str = "info", duration: int = 3000):  # noqa: D401
        """Alias Wrapper f??r show_toast (Namensvereinheitlichung, keine Icons)."""
        try:
    ...
    # Nur System-Gesamtstatus darf weiterhin gr??n sein. Alles andere:
    #  - Erfolg -> primary
    #  - Warnung -> primary oder text_secondary (je nach Domain)
    #  - Info -> primary
    #  - Zerst??rerisch / Danger bleibt error (rot)
    # -------------------------------------------------------------
    def _accent(self, semantic: str, domain: str = "generic"):
        try:
            if semantic == 'success':
                if domain == 'system_status':  # einzig erlaubtes persistent Gr??n
    ...
    def create_project_structure(self, customer_name, project_date=None):
        """???? Erstellt vollst??ndige Projektstruktur nach bestehender Checker-Ordnerstruktur"""
        try:
            import datetime
            if not project_date:
                project_date = datetime.datetime.now().strftime("%Y-%m-%d")
    ...
    def validate_project_structure(self, project_path):
        ...
        try:
            ...
            if missing_folders:
                self.logger.info(f"???? Reparierte Ordnerstruktur: {len(missing_folders)} Ordner hinzugef??gt")
            
            return len(missing_folders) == 0
            
        except Exception as e:
            self._handle_error(e, context="project.structure.validate", user_message=self._t("Projektstruktur ung??ltig"), toast=False)
            return False
    
    def get_project_paths(self, customer_name, project_date=None):
        """???? Gibt alle wichtigen Projektpfade nach bestehender Struktur zur??ck"""
        try:
            import datetime
            if not project_date:
                project_date = datetime.datetime.now().strftime("%Y-%m-%d")
            ...
            return {
                ...
                'pr??fung': os.path.join(base_path, "03_Pr??fung"),
    ...
    def _get_or_select_customer_for_upload(self):
        ...
        try:
            ...
            if customer_name:
                # Kundennamen bereinigen f??r Ordnernamen
                import re
    ...
    def _offer_open_project_folder(self, customer_name):
        """???? Bietet an, das Projekt-Verzeichnis zu ??ffnen"""
        try:
            import datetime
            project_date = datetime.datetime.now().strftime("%Y-%m-%d")
            
            # Toast mit Projekt-Ordner ??ffnen Option
            message = f"Projekt '{customer_name}' aktualisiert. Ordner ??ffnen?"
            
            # Erweiterte Toast-Nachricht mit Action
            if hasattr(self, 'extended_toast_system') and self.extended_toast_system:
                # Falls erweiterte Toast verf??gbar, nutze diese f??r interaktive Nachrichten
                self.root.after(1000, lambda: self.extended_toast_system.show_toast(
                    message, 
                    "success", 
                    4000,
                    action_text="Ordner ??ffnen",
    ...
    def _open_specific_project(self, customer_name, project_date):
        """???? ??ffnet spezifisches Projekt im Datei-Explorer"""
        try:
            ...
            if sys.platform.startswith('win'):
                project_path = os.path.normpath(project_path)  # ??? Normalisiert f??r Explorer
            ...
            try:
                ...
                self.logger.info(f"Projekt-Ordner ge??ffnet: {project_path}")
            except FileNotFoundError as fe:
                self._handle_error(fe, context="project.folder.open.cmd", user_message=self._t("??ffnen-Kommando nicht gefunden") if hasattr(self, '_t') else None)
                self.show_toast("??ffnen-Kommando fehlt", "error")
            except Exception as se:
                self._handle_error(se, context="project.folder.open.exec", user_message=self._t("Projekt-Ordner konnte nicht ge??ffnet werden") if hasattr(self, '_t') else None)
                self.show_toast("Fehler beim ??ffnen des Ordners", "error")
        except Exception as e:
            self._handle_error(e, context="project.folder.open", user_message=self._t("Projekt-Ordner konnte nicht ge??ffnet werden") if hasattr(self, '_t') else None)
            try:
                self.show_toast("Fehler beim ??ffnen des Ordners", "error")
            except Exception:
                pass
    ...
    def _show_professional_customer_dialog(self, existing_customers):
        ...
        try:
            ...
            # R??ckgabewert f??r selected customer
            ...
            title_label = ctk.CTkLabel(
                header_frame,
                text="Kunde f??r Projekt ausw??hlen",
                font=ctk.CTkFont(*self.get_typography("heading")),
                text_color=self.get_color('gray_700')
            )
            ...
            subtitle_label = ctk.CTkLabel(
                header_frame,
                text="W??hle einen Kunden f??r Projektstruktur oder ??berspringe f??r einfachen Upload",
                font=ctk.CTkFont(*self.get_typography("body")),
                text_color=self.get_color('gray_500')
            )
            ...
            if existing_customers:
                ...
                # Scrollbarer Bereich f??r Kunden
            ...
            # Enter-Taste f??r Kunden erstellen
            ...
            # - None: Benutzer hat ??bersprungen ??? Upload ohne Projektstruktur
            # - String: Kunde ausgew??hlt ??? Upload mit Projektstruktur
            result = selected_customer or self._temp_selected_customer
            if result == "CANCELLED":
                return "CANCELLED"  # Spezialwert f??r Abbruch
            return result  # None f??r ??berspringen, String f??r Kunde
    ...
    def _is_source_text(self, file_path):
        """???? Klassifiziert ob Datei ein Ausgangstext ist (f??r 01_Ausgangstext)"""
        # Alle hochgeladenen Dateien gehen standardm????ig in 01_Ausgangstext
        return True
    
    def _is_translation(self, file_path):
        """???? Klassifiziert ob Datei eine ??bersetzung ist (nicht relevant f??r diese Struktur)"""
        # In der neuen Struktur sind ??bersetzungen Teil des Workflows, nicht separate Uploads
        return False
    ...
    def copy_files_to_project_structure(self, files, customer_name, project_date=None):
        ...
        try:
            ...
            self._handle_error(e, context="project.copy", user_message=self._t("Projekt-Ordner konnte nicht ge??ffnet werden") if hasattr(self, '_t') else "Projekt-Ordner Fehler", toast=False)
            return {}
    
    def _upload_source_files(self):
        """Delegierter Quelldatei-Upload ??ber UploadService (GUI bleibt schlank)."""
        try:
            file_types = [
            ...
            files = filedialog.askopenfilenames(title="Ausgangstexte f??r Qualit??tsanalyse ausw??hlen", filetypes=file_types)
            if not files:
                return
            ...
            if added:
                ...
                self.show_toast("Alle ausgew??hlten Dateien waren bereits hochgeladen", "info")
    ...
    def _upload_batch_files(self):
        """Delegierter Batch Upload ??ber UploadService."""
        try:
            file_types = [
            ...
            files = filedialog.askopenfilenames(title="Dateien f??r Stapel-Upload und Analyse ausw??hlen", filetypes=file_types)
            if not files:
                return
            ...
            self._schedule_update_file_counter()
            # Telemetrie f??r hinzugef??gte Dateien (je Typ separat)
            try:
                if getattr(self, 'event_bus', None):
                    after_total = sum(len(self.uploaded_files.get(k, [])) for k in ('source','translation'))
                    if result['source_added']:
                    ...
                    if result['translation_added']:
                        self.event_bus.publish('files.changed', {
                            ...
                            'before_total': batch_before_total,  # gleicher before_total Kontext f??r Batch
                            'after_total': after_total
                        })
            except Exception:
                pass
            ...
            if total_added > 0:
                status_msg = f"Batch Upload abgeschlossen: {total_added} Dateien hinzugef??gt"
                if skipped_files:
                ...
                self.update_status("Keine neuen Dateien hinzugef??gt", "warning")
                self.show_toast("Keine neuen Dateien hinzugef??gt", "warning")
    ...
    def _show_batch_upload_results(self, source_count, translation_count, processed_files, skipped_files):
        ...
        try:
            ...
            # Adaptive H??he basierend auf Anzahl-Eintr??ge
            ...
            ctk.CTkLabel(
                translation_stat,
                text=self._t("??bersetzungen") if hasattr(self, '_t') else "??bersetzungen",
                font=ctk.CTkFont(*self.get_typography('caption')),
                text_color=self.get_color('info')
            ).pack(pady=(0, 10))
            ...
            ctk.CTkLabel(
                skipped_stat,
                text=self._t("??bersprungen") if hasattr(self, '_t') else "??bersprungen",
                font=ctk.CTkFont(*self.get_typography('caption')),
                text_color=self._accent('warning')
            ).pack(pady=(0, 10))
            ...
            if processed_files or skipped_files:
                ...
                if processed_files:
                    ctk.CTkLabel(
                        ...
                        font=ctk.CTkFont(*self.get_typography('subheading')),
                        # Accent Reduction: keine direkte success-Farbe hier (nur System-Status darf gr??n sein)
                ...
                if skipped_files:
                    ctk.CTkLabel(
                        details_frame,
                        text=self._t("??bersprungene Dateien:") if hasattr(self, '_t') else "??bersprungene Dateien:",
                        font=ctk.CTkFont(*self.get_typography('subheading')),
                        text_color=self._accent('warning'),
                        anchor="w"
            ...
            close_btn = self._create_button(
                content_frame,
                text=self._t("Schlie??en") if hasattr(self, '_t') else "Schlie??en",
    ...
    def _setup_modern_file_management(self, parent):
        ...
        try:
            ...
            self.clear_files_btn = self._create_button(
                action_frame,
                text=(self._t("Alles l??schen") if hasattr(self,'_t') else "Alles l??schen"),
    ...
    def _create_file_list_section(self, parent, title, file_type, column):
        ...
        try:
            ...
            # Mehr Au??en-Padding f??r den Header
            ...
            # Mehr Innen-Abstand zwischen Rahmen und Scrollfl??che
    ...
    def _update_file_counter(self):
        """???? KONSOLIDIERT: Aktualisiert ALLE Dateiz??hler - Header und Card"""
        try:
            start_ts = time.time()
            source_count = len(self.uploaded_files.get('source', []))
            translation_count = len(self.uploaded_files.get('translation', []))
            ...
            if source_count > 0 and translation_count > 0:
                ...
                status_text = f" ??? {pairs} Paar(e) bereit f??r Analyse"
                status_color = self._accent('success', 'system_status')
                if hasattr(self, 'readiness_indicator'):
                    self.readiness_indicator.configure(text="??? Bereit f??r Analyse", text_color=self._accent('success', 'system_status'))
            elif source_count > 0 or translation_count > 0:
                if source_count > translation_count:
                    missing = source_count - translation_count
                    status_text = f" ??? {missing} weitere ??bersetzungsdatei(en) hochladen"
                else:
            ...
            counter_text = f"Dateien: {source_count} Ausgangstexte, {translation_count} ??bersetzungen{status_text}"
            simple_counter_text = f"Dateien: {source_count} Ausgangstexte, {translation_count} ??bersetzungen"
            ...
            if hasattr(self, 'translation_count_label'):
                self.translation_count_label.configure(text=f"{translation_count} ??bersetzungen")
            ...
            if hasattr(self, 'analyze_button'):
                if source_count > 0 and translation_count > 0:
                    self.analyze_button.configure(
                        state="normal",
                        fg_color=self.get_color('primary'),
                        text=(self._t("Qualit??tsanalyse starten") if hasattr(self, '_t') else "Qualit??tsanalyse starten")
                    )
                else:
                    self.analyze_button.configure(
                        state="disabled",
            ...
            try:
                if hasattr(self, 'header_file_counter_label'):
                    self.header_file_counter_label.configure(text="Dateien: Fehler beim Z??hlen")
                if hasattr(self, 'card_file_counter_label'):
                    self.card_file_counter_label.configure(text="Dateien: Fehler beim Z??hlen")
            except Exception:
                pass
            try:
            ...
            self._handle_error(
                e,
                context="files.counter.update",
                user_message=(self._t("Dateiz??hler konnte nicht aktualisiert werden") if hasattr(self,'_t') else "Dateiz??hler konnte nicht aktualisiert werden"),
                toast=False
            )
    ...
    def _schedule_update_file_counter(self, delay_ms: int = 80):
        ...
        Mehrere schnelle ??nderungen (Uploads/Removals) l??sen nur einen
        tats??chlichen Z??hler-/Pairing-Update nach Ablauf von delay_ms aus.
        F??llt auf direkten Aufruf zur??ck falls root fehlt.
        """
        try:
    ...
    def _publish_files_state(self, source_count: int, translation_count: int, total_count: int, duration_ms: int, ts: float):
        ...
        Liefert zus??tzlich diff zu vorherigem Snapshot.
        """
        try:
    ...
    def _update_file_list_content(self, file_type, list_frame, empty_msg):
        ...
        try:
            ...
            for widget in list_frame.winfo_children():
                if widget != empty_msg:  # ??? Empty-Message nicht zerst??ren
                    widget.destroy()
    ...
    def _format_file_size(self, size_bytes):
        """DATEIGR??SSE FORMATIEREN (Delegation) - Single Source of Truth.

        Diese fr??here lokale Implementierung delegiert jetzt an die zentrale
        ...
        try:
            ...
            # Minimaler Fallback falls Import fehlschl??gt
            try:
                b = int(size_bytes)
                if b < 1024:
                    return f"{b} B"
    ...
    def _remove_file(self, file_type, index):
        """??????? REMOVE FILE - Remove specific file from list"""
        try:
            files = self.uploaded_files.get(file_type, [])
            ...
            if hasattr(self, 'pairing_status_label'):
                ...
                if pairs_count > 0:
                    ...
                    self.pairing_status_label.configure(
                        text=status_text,
                        # Accent Reduction: Paarungs-Status nutzt jetzt Primary statt success (Gr??n reserviert f??r system_status)
                        text_color=self._accent('success')
                    )
                else:
                    self.pairing_status_label.configure(
    ...
    def _normalize_filename(self, filepath):
        """Delegiert an PairingService f??r Normalisierung (Backward Compatibility Stub)."""
        try:
    ...
    def _display_file_pairing_results(self, pairs, unmatched_source, unmatched_translation):
        ...
        try:
            ...
            if pairs:
                ...
                for pair in pairs:
                    self.logger.info(f"???? Pair: {pair['source_name']} ??? {pair['translation_name']} (??hnlichkeit: {pair['similarity']:.1%})")
            else:
    ...
    def get_file_pairs(self):
        """???? GET FILE PAIRS - Hole aktuelle Dateipaarung f??r Qualit??tsanalyse"""
    ...
    def _check_and_show_manual_pairing_option(self):
        """???? CHECK MANUAL PAIRING OPTION - Pr??fe ob manuelles Pairing angeboten werden soll"""
        try:
            source_count = len(self.uploaded_files.get('source', []))
            translation_count = len(self.uploaded_files.get('translation', []))
            ...
            # 2. UND (ungepaarte Dateien existieren ODER Benutzer m??chte manuell konfigurieren)
            if source_count > 0 and translation_count > 0:
                unmatched_total = 0
                if hasattr(self, 'unmatched_files'):
    ...
    def _show_manual_pairing_dialog(self):
        ...
        try:
            ...
            # Cleanup Handler f??r Scroll-Bindings
            ...
            try:
                def _confirm_close_pairing_window():
                    ...
                    if dirty:
                        try:
                            self._confirm(
                                title=self._t('Discard changes?') if hasattr(self,'_t') else '??nderungen verwerfen?',
            ...
            instructions = ctk.CTkLabel(
                content_frame,
                text="Verbinde Ausgangstexte mit ihren ??bersetzungen durch Ziehen oder Dropdown-Auswahl:",
                font=ctk.CTkFont(*self.get_typography('body')),
                text_color=self.get_color('text_primary')
            )
            ...
            self.show_toast("Fehler beim ??ffnen der manuellen Paarung", "error")
    ...
    def _setup_manual_pairing_interface(self, parent, window):
        ...
        try:
            ...
            translation_label = ctk.CTkLabel(
                ...
                text=self._t("Unpaired translations") if hasattr(self,'_t') else "Ungepaarte ??bersetzungen",
                font=ctk.CTkFont(*self.get_typography('body_bold')),
                text_color=self.get_color('text_primary')
            )
            ...
            # Ermittele Toplevel f??r sichere Save/Cancel Aktionen
            try:
            ...
            # Cleanup MouseWheel Bindings beim Schlie??en (falls bind_all genutzt wurde)
            try:
                def _cleanup_bindings():
                    try:
            ...
            self._handle_error(e, context="pairing.manual.interface", user_message=self._t("Manuelle Pairing Oberfl??che Fehler"))

    def _populate_manual_pairing_interface(self):
        """POPULATE MANUAL PAIRING INTERFACE - F??lle Interface mit aktuellen Daten"""
        try:
            self.logger.debug("???? Populating manual pairing interface???")
            start_time = time.perf_counter()
            created_items = {'source': 0, 'translation': 0}
            # Telemetrie-Z??hler initialisieren (lazy)
            if not hasattr(self, '_telemetry_counters'):
                self._telemetry_counters = {}
            ...
            # Wenn keine Daten vorhanden sind, f??hre Smart Pairing aus
            ...
            if not hasattr(self, '_pairing_toolbar'):
                ...
                try:
                    ...
                    sort_cb = ctk.CTkCheckBox(
                        toolbar,
                        text=self._t("Sort by similarity") if hasattr(self,'_t') else "Nach ??hnlichkeit sortieren",
            ...
            # Virtuelle Fenstergr????en Parameter (dynamisch versuchen abzuleiten)
            # Heuristik: Sch??tze wie viele Items vertikal passen anhand verf??gbarer H??he der Source-Frame
            try:
                available_height = self.unmatched_source_frame.winfo_height()
            except Exception:
                available_height = 0
            # Wenn Frame noch nicht gerendert ist (H??he=0) fallback auf vorher gespeicherten Wert oder Default
            ...
            # Typische Item-H??he inkl. Padding gesch??tzt: 34px (Label/Frame + Spacing). Sicherheitskorridor 36.
            est_item_height = 36
            ...
            # Falls Fenstergroesse sich ge??ndert hat, aktualisieren
            if vp.get('window') != VWINDOW:
                vp['window'] = VWINDOW
            # Korrigiere Offsets falls sich Gesamtzahl ge??ndert hat
            ...
            def render_window(frame, files, ftype, offset_key):
                """Render ein virtuelles Fenster f??r einen Dateityp."""
                try:
            ...
            # Mousewheel Scroll Binding f??r dynamische Offsets
            ...
            if paged:
                ...
                # ???? Komponiere Text ohne Annahme ??ber Platzhalter im ??bersetzungs-String
                if hasattr(self, '_t'):
                    sources_label = self._t('sources') if hasattr(self,'_t') else 'Quellen'
                    translations_label = self._t('translations') if hasattr(self,'_t') else '??bersetzungen'
                    btn_text = (self._t("Show all") + f" ({total_sources} {sources_label} / {total_trans} {translations_label})")
                else:
                    btn_text = f"Alle anzeigen ({total_sources} Quellen / {total_trans} ??bersetzungen)"
            ...
            try:
                ...
                pass
            # Nach initialem Render dynamische Justierung (nur wenn Frame-H??hen vorher 0 waren)
            try:
            ...
            self._handle_error(e, context="pairing.manual.populate", user_message=self._t("Manuelle Pairing Bef??llung Fehler"), toast=False)
        finally:
            try:
                if hasattr(self, '_update_undo_redo_buttons'):
                    self._update_undo_redo_buttons()
    ...
    def _adjust_virtual_window_size_safe(self):
        """Passe die virtuelle Fenstergr????e adaptiv an reale Item-H??hen an (Heuristik)."""
        try:
            ...
            # Durchschnittliche H??he der ersten bis zu 10 Items (exkl. Label & Navigationsbuttons erkennbar an f??hrenden Symbolen)
            item_heights = []
    ...
    def _create_pair_display_item(self, parent, pair, index):
        ...
        try:
            ...
            # Dynamische Farbgebung nach ??hnlichkeitswert
    ...
    def _create_unmatched_file_item(self, parent, file_path, file_type):
        ...
        try:
            ...
            # Dropdown f??r manuelle Paarung
            if file_type == "source":
                # Zeige verf??gbare Translation-Dateien
                ...
                if available_translations:
                    ...
                    # Keine verf??gbaren ??bersetzungen
                    no_trans_label = ctk.CTkLabel(
                        content_frame,
                        text=self._t("No unmatched translations available") if hasattr(self,'_t') else "Keine ungepaarten ??bersetzungen verf??gbar",
                        font=ctk.CTkFont(*self.get_typography('caption')),
                        text_color=self.get_color('text_secondary')
                    )
                ...
                # Zeige verf??gbare Source-Dateien
                ...
                if available_sources:
                    ...
                    # Keine verf??gbaren Quellen
                    no_source_label = ctk.CTkLabel(
                        content_frame,
                        text=self._t("No unmatched sources available") if hasattr(self,'_t') else "Keine ungepaarten Ausgangstexte verf??gbar",
                        font=ctk.CTkFont(*self.get_typography('caption')),
                        text_color=self.get_color('text_secondary')
                    )
    ...
    def _manual_pair_files(self, source_file, selected_translation, file_type):
        ...
        try:
            ...
            # Finde vollst??ndigen Pfad der ausgew??hlten Translation
            if hasattr(self, 'unmatched_files') and self.unmatched_files.get('translation'):
                available_translations = self.unmatched_files.get('translation', [])
            else:
                # Fallback
    ...
    def _manual_pair_files_translation(self, translation_file, selected_source, file_type):
        ...
        try:
            ...
            # Finde vollst??ndigen Pfad der ausgew??hlten Source
            if hasattr(self, 'unmatched_files') and self.unmatched_files.get('source'):
                available_sources = self.unmatched_files.get('source', [])
            else:
                # Fallback
    ...
    def _unpair_files(self, pair_index):
        """UNPAIR FILES - L??se Dateipaarung auf"""
        try:
    ...
    def _reset_to_automatic_pairing(self):
        """??? RESET TO AUTOMATIC PAIRING - Setze auf automatische Paarung zur??ck"""
        try:
            # History Snapshot (einfacher) ??? vollst??ndige Zust??nde bereits im History Stack
            self._push_history()
            # F??hre automatische Paarung erneut aus
            self._smart_file_pairing()
    ...
    # ------------------------------------------------------------------
    # UNDO / REDO PAIRING SYSTEM (additiv, keine bestehende Signatur ge??ndert)
    # ------------------------------------------------------------------
    def _record_pair_action(self, action_type: str, data: Dict[str, Any]):
        """Interne History-Aufzeichnung f??r Pairing Aktionen.
        action_type: create_pair | remove_pair | reset_auto_pairing
        data: kontextabh??ngige Daten (siehe Aufrufer)
        """
        if not hasattr(self, '_pair_history'):
    ...
    def _undo_pair_action(self):
        """DEPRECATED: Weiterleitung auf neues _undo System (Kompatibilit??t)."""
        return self._undo()

    def _redo_pair_action(self):
        """DEPRECATED: Weiterleitung auf neues _redo System (Kompatibilit??t)."""
        return self._redo()
    ...
    def _shift_virtual_window(self, offset_key: str, delta: int):
        """Verschiebe das virtuelle Fenster f??r Unmatched-Listen."""
        try:
            if not hasattr(self, '_virtual_pairing'):
                return
            vp = self._virtual_pairing
    ...
    def _begin_pair_drag(self, file_path: str, file_type: str, widget):
        """Starte Drag f??r eine ungepaarte Datei."""
        try:
    ...
    def _drag_enter_item(self, target_file: str, target_type: str, widget):
        """Hover ??ber potentielles Drop-Ziel."""
        try:
    ...
    def _complete_pair_drag(self, release_file: str, release_type: str):
        """Abschluss eines Drag & Drop Pairings, falls m??glich."""
        try:
            if not self._pairing_drag:
                return
            ...
            try:
                ...
                pass
            # Snapshot vor ??nderung f??r Undo/Redo System
            try:
                if hasattr(self, '_push_history'):
                    self._push_history()
            except Exception:
                pass
            # F??hre Pairing durch (nutzt bestehende Logik f??r Konsistenz)
            new_pair = {
                'source': source_file,
                'translation': translation_file,
                'similarity': 1.0,
    ...
    def _ctx_menu_for(self, fpath):
        try:
            ...
            menu.add_command(label=self._t('Open') if hasattr(self,'_t') else '??ffnen', command=lambda: os.startfile(fpath) if os.name=='nt' else None)
    ...
    def _clear_all_files(self):
        """CLEAR ALL FILES - Alle hochgeladenen Dateien l??schen"""
        try:
            total_files = len(self.uploaded_files.get('source', [])) + len(self.uploaded_files.get('translation', []))
            
            if total_files == 0:
                self.show_toast("Keine Dateien zum L??schen vorhanden", "info")
                return
            
            # Alle Dateien l??schen
            self.uploaded_files = {'source': [], 'translation': []}
            
            # File pairs und unmatched files auch l??schen
            if hasattr(self, 'file_pairs'):
                self.file_pairs = []
            if hasattr(self, 'unmatched_files'):
                self.unmatched_files = {'source': [], 'translation': []}
            ...
            self.update_status("Alle Dateien wurden gel??scht")
            self.show_toast(f"{total_files} Dateien erfolgreich gel??scht", "success")
            # Event publizieren
            try:
                if getattr(self, 'event_bus', None):
            ...
            try:
                self._handle_error(e, context="files.clear_all", user_message=self._t("Dateien L??schen Fehler"))
            except Exception:
                pass
            self.show_toast("Fehler beim L??schen der Dateien", "error")
    ...
    def _refresh_file_list(self):
        ...
        try:
            ...
            total_files = len(self.uploaded_files.get('source', [])) + len(self.uploaded_files.get('translation', []))
            self.update_status(f"Dateiliste aktualisiert - {total_files} Dateien verf??gbar")
    ...
    def _upload_translation_files(self):
        """Delegierter ??bersetzungsdatei-Upload via UploadService."""
        try:
            file_types = [
                ("All Supported", "*.pdf;*.docx;*.txt;*.doc;*.rtf;*.odt"),
                ("PDF files", "*.pdf"),
            ...
            files = filedialog.askopenfilenames(title="??bersetzungsdateien f??r Qualit??tsanalyse ausw??hlen", filetypes=file_types)
            if not files:
                return
            customer_name = self._get_or_select_customer_for_upload()
            if customer_name == "CANCELLED":
            ...
            if added:
                ...
                status_msg = f"{len(added)} ??bersetzungsdateien hinzugef??gt"
                if duplicates:
                    status_msg += f" ({len(duplicates)} Duplikate ignoriert)"
                if source_count > 0 and trans_count > 0:
                    status_msg += f" | Bereit f??r Analyse ({min(source_count, trans_count)} Paare)"
                self.update_status(status_msg)
                self.show_toast(f"{len(added)} ??bersetzungsdateien erfolgreich hochgeladen", "success")
                if source_count > 0 and trans_count > 0:
                    self.root.after(1000, lambda: self.show_toast("Bereit f??r Qualit??tsanalyse!", "info"))
            ...
            msg = f"Fehler beim Hochladen der ??bersetzungsdateien: {e}"
            self.update_status(msg)
            self.show_toast("Fehler beim Hochladen der ??bersetzungsdateien", "error")
            try:
                self._handle_error(e, context="upload.translation", user_message=self._t("??bersetzungsdateien Upload fehlgeschlagen"))
            except Exception:
                pass
...
??? Gesamtgr????e: {self._format_file_size(total_size)}
...
"""
    ...
    def _format_file_size(self, size_bytes):
        # Diese zweite fr??here Doppel-Definition wurde entfernt (Consolidation).
        # Beibehalten f??r R??ckw??rtskompatibilit??t falls Referenzen bestehen.
        try:
            from quality_gui_utilities import format_file_size
            return format_file_size(int(size_bytes) if size_bytes is not None else 0)
        except Exception:
    ...
    def _get_smart_upload_advice(self, file_type):
        """INTELLIGENT: Kontextuelle Ratschl??ge basierend auf Upload-Status"""
        try:
            source_count = len(self.uploaded_files['source'])
            trans_count = len(self.uploaded_files['translation'])
            ...
            if file_type == "source":
                if trans_count == 0:
                    return "N??chster Schritt: Entsprechende ??bersetzungsdateien hochladen"
                elif trans_count < source_count:
                    return f"Ausgleich: Erw??gen Sie das Hochladen von {source_count - trans_count} weiteren ??bersetzungsdatei(en)"
                else:
                    return "Bereit: Alle Dateien hochgeladen - Qualit??tsanalyse starten!"
            else:  # translation
                if source_count == 0:
                    return "N??chster Schritt: Entsprechende Ausgangstexte hochladen"
                elif source_count < trans_count:
                    return f"Ausgleich: Erw??gen Sie das Hochladen von {trans_count - source_count} weiteren Ausgangstexten"
                else:
                    return "Bereit: Alle Dateien hochgeladen - Qualit??tsanalyse starten!"
        except:
    ...
    def start_analysis(self, rule_profile: str = "default"):
        """Startet Analyse mit Cache-Pr??fung und Event-Publishing."""
        try:
                    ...
                    if cached:
                        ...
                        try:
                            if hasattr(self, '_show_analysis_results'):
                                # Markiere current_analysis f??r Kompatibilit??t
            ...
            # Listener f??r progress Events (einmalig registrieren)
            ...
            def job():
                ...
                except Exception as e:  # Einheitliche Fehlerbehandlung ??ber zentralen Handler
                    duration_ms = int((time.time() - start_ts) * 1000)
                    try:
    ...
    def _simulate_analysis_progress(self, progress_bar, status_label):  # pragma: no cover
        """Legacy Simulation deaktiviert ??? beibehalten f??r R??ckw??rtskompatibilit??t."""
        try:
...
"""
    ...
    def export_results(self):
        ...
        try:
            ...
            export_text = self._t("Verf??gbare Export-Formate:") + "\n\n" + "\n".join([
                self._t("PDF Bericht (Empfohlen)"),
                "   ??? " + self._t("Umfassende Qualit??tsanalyse"),
            ...
            def _add_btn(text, fmt):
                # Nutzung der Factory f??r konsistente Export-Buttons
            ...
            # ???? Zusatz: HTML Bericht direkt ??ffnen (Additiv, keine bestehende Logik ver??ndert)
            try:
                report_btn = _add_btn(self._t("Bericht ??ffnen"), 'html')
                # ??berschreibe Command f??r HTML speziell ??? kein Export, sondern Anzeige
            ...
            self.show_toast(self._t("Export Funktionalit??t bereit"), "info")
    ...
    def _perform_export(self, fmt: str):
        ...
        try:
            ...
            try:
                self._handle_error(e, context="export.module.missing", user_message=self._t("Export Modul nicht verf??gbar"))
            except Exception:
                pass
            self.show_toast("Export-Modul nicht verf??gbar", "error")
            return
        try:
            import uuid, time as _tmod
            ...
            # Wenn Button explizit Format liefert, ??berschreibe Auswahl (User Intent)
            ...
            # prefix bleibt f??r R??ckw??rtskompatibilit??t als zus??tzliches Pr??fix
    ...
    # ---------------------------------------------------------------
    # ???? HTML REPORT INTEGRATION (Additiv) ??? ??ffnet migrierte Bericht_*.html Dateien
    # ---------------------------------------------------------------
    def _open_quality_report(self):
        """??ffnet einen vorhandenen migrierten HTML-Bericht (Bericht_*.html) im Standardbrowser.
        ...
        try:
            ...
            # 2. Statische migrierte Varianten (Fallback Reihenfolge unver??ndert, aber nach dynamisch)
            candidates.extend([
            ...
            if opened:
                ...
                if target.endswith('Bericht_Dynamisch.html'):
                    self.show_toast(self._t("Dynamischer Bericht ge??ffnet"), "success")
                else:
                    self.show_toast(self._t("Bericht ge??ffnet"), "success")
            else:
                self.show_toast(self._t("Bericht konnte nicht ge??ffnet werden"), "error")
        except Exception as e:
            try:
                self._handle_error(e, context="report.open", user_message=self._t("??ffnen des Berichts fehlgeschlagen"))
            except Exception:
                pass
    ...
    def clear_files(self):
        ...
        try:
            ...
            # Direkte Ausf??hrung bei kleiner Menge
    ...
    def _show_clear_files_dialog(self, total_files: int, source_count: int, translation_count: int):
        """Nicht-blockierender Best??tigungsdialog f??r Dateil??schung."""
        try:
            dialog = ctk.CTkToplevel(self.root)
            dialog.title(self._t("Best??tigung"))
            dialog.transient(self.root)
            dialog.grab_set()
            try:
                dialog.attributes('-topmost', True)
            ...
            title_lbl = ctk.CTkLabel(frame, text=self._t("Dateien l??schen"), font=ctk.CTkFont(*self.get_typography('subheading')) if hasattr(self,'get_typography') else None, text_color=self.get_color('text_primary') if hasattr(self,'get_color') else None)
            title_lbl.pack(anchor='w', pady=(4,8))

            msg = self._t("{n} Dateien l??schen? Dies kann nicht r??ckg??ngig gemacht werden.").format(n=total_files)
            detail = self._t("Quellen: {s} ?? ??bersetzungen: {t}").format(s=source_count, t=translation_count)
            ...
            def _cancel():
                try:
                    ...
                    pass
                self.update_status(self._t("L??schung abgebrochen"))

            confirm_btn = self._create_button(btn_row, text=self._t("L??schen"), command=_confirm, kind="danger", height=36)
    ...
    def _execute_clear_files(self, total_files: int):
        """Interne Ausf??hrung der Dateil??schung (aus Dialog oder direkt)."""
        try:
            self.uploaded_files = {'source': [], 'translation': []}
            ...
            try:
                self._handle_error(e, context='files.clear.execute', user_message=self._t('Dateil??schung Fehler'))
            except Exception:
                pass
    ...
    def show_settings_view(self):
        ...
        try:
            ...
            try:
                ...
                auto_open_toggle = ctk.CTkCheckBox(reporting_section, text="Bericht nach Export automatisch ??ffnen", variable=auto_open_var, onvalue=True, offvalue=False, fg_color=self.get_color('primary'))
                ...
                prefix_label = ctk.CTkLabel(prefix_frame, text="Dateipr??fix", font=ctk.CTkFont(*self.get_typography('caption')))
                ...
                def _save_reporting_settings():
                    try:
                        if hasattr(self, 'settings_service') and self.settings_service:
                            ...
                            self.show_toast(self._t("SettingsService nicht verf??gbar"), "error")
                    except Exception as e:
            ...
            try:
                ...
                ui_label = ctk.CTkLabel(ui_section, text=self._t("Benutzeroberfl??che"), font=ctk.CTkFont(*self.get_typography('label_bold')), text_color=self.get_color('text_primary'))
                ...
                def _toggle_contrast():
                    try:
                        ...
                        # ThemeGuard re-evaluiert ??ber provider lambda -> einfach repaint
...
"""
    ...
    def _run_background_analysis_safe(self, analysis_id: str):
        ...
        Erzeugt (optionalen) Report unabh??ngig von UI-Simulation.
        ...
        try:
            ...
            try:
                ...
                pass
            # Dynamische Score Absch??tzung: Durchschnitt vorhandener plugin score Felder falls vorhanden
            dynamic_score = 87.0
            try:
                numeric_scores = []
                for r in plugin_results:
    ...
    def _export_and_publish_report(self, report, background: bool = False):
        """Zentrale Export-/Event-Pipeline f??r Reports (wiederverwendet UI & Background)."""
        try:
    ...
    def _prompt_translation(self):
        """Einheitlicher Prompt-Text f??r ??bersetzungs-Dropdowns."""
        try:
    ...
    def _prompt_source(self):
        """Einheitlicher Prompt-Text f??r Source-Dropdowns."""
        try:
    ...
    def _label_source(self):
        """Lokalisierter Label-Text f??r Source Dateien."""
        try:
    ...
    def _label_translation(self):
        """Lokalisierter Label-Text f??r Translation Dateien."""
        try:
    ...
    def _repaint_colors_safe(self):
        try:
            # Primitive: Alle Frames & Labels neu einf??rben (nur sichtbare Hauptbereiche)
            if not self.root:
                return
            ...
            pass
                    ...
                    # (Entfernt: versehentlich duplizierter files.changed Block au??erhalb Funktionen)
    def _repaint_fonts_safe(self):
        try:
            if not self.root:
                return
            # Aktualisiere nur Labels ohne bereits gesetzte gr????ere Typografie (heuristisch: Textl??nge < 60 oder Font-Cache Flag?)
    ...
    def _log_event(self, name: str, **payload):
        """Strukturiertes Event-Logging (JSON-basiert f??r robustere Werte)."""
        try:
            import json
