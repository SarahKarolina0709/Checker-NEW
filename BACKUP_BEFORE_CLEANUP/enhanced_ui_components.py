...
class ModernDropZone(ctk.CTkFrame):
    ...
    def setup_interactions(self):
        ...
        # Drag & Drop (falls tkinterdnd2 verfügbar)
        try:
    ...
    def _on_drop(self, event):
        """Behandelt Datei-Drops"""
        files = event.data.split()
        valid_files = []
        
        for file in files:
            if any(file.lower().endswith(ext) for ext in self.accepted_files):
                valid_files.append(file)
        
        if valid_files and len(valid_files) <= self.max_files:
