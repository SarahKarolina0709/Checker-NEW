...
class PruefungWorkflowController:
    ...
    def __init__(self, project_data=None, app=None):
        ...
        self.language_var = None  # Will be set by the view
        ...
        self.selected_file_pair_id = None        # Check selection management (will be initialized when view is set)
        self.selected_checks = {}
        
        # Initialize language tool if available
        if LANGUAGE_TOOL_AVAILABLE:
            self._initialize_language_tool_async("de-DE")
      def set_view(self, view):
        """Set the view reference."""
        self.view = view
        # Connect to the view's language variable
        if hasattr(view, 'language_var'):
            self.language_var = view.language_var
        
        # Initialize check selection variables now that we have a tkinter context
    ...
    def get_tab_configurations(self):
        """Get tab configurations for the results display."""
        return {
            "errors": {
                "title": "🔍 Grammatik & Rechtschreibung",
                "color": "#FF6B6B"
            },
            "quality": {
    ...
    def on_language_change(self, *args):
        """Handle language change events from the UI."""
        try:
            if hasattr(self, 'language_var') and self.language_var:
                selected_language = self.language_var.get()
                print(f"[DEBUG] Language changed to: {selected_language}")
                self.language = selected_language
                
                # Reinitialize LanguageTool with new language if available
                if LANGUAGE_TOOL_AVAILABLE:
                    lang_code = "de-DE" if selected_language == "Deutsch" else "en-US"
                    self._initialize_language_tool_async(lang_code)
        except Exception as e:
            print(f"[ERROR] Error in on_language_change: {e}")
    ...
    def _initialize_language_tool_async(self, lang_code):
